...
export const klients = pgTable('klients', {
  ...
  passwordHash: text('password_hash'), // null = password not set; klient can still use magic link
...
// ============================================================================
// SESSIONS — server-side session store; cookie holds opaque session id
// ============================================================================
export const sessions = pgTable(
  'sessions',
  {
    id: text('id').primaryKey(), // 32-byte hex, generated by app
    klientId: uuid('klient_id')
      .notNull()
      .references(() => klients.id, { onDelete: 'cascade' }),
  ...
  (table) => ({
    klientIdx: index('sessions_klient_idx').on(table.klientId),
    expiresIdx: index('sessions_expires_idx').on(table.expiresAt),
  }),
);

// ============================================================================
// MAGIC LINK TOKENS — single-use passwordless login tokens
// ============================================================================
export const magicLinkTokens = pgTable(
  'magic_link_tokens',
  {
    ...
    klientId: uuid('klient_id')
      .notNull()
      .references(() => klients.id, { onDelete: 'cascade' }),
    tokenHash: text('token_hash').notNull(), // sha256 of the URL token; URL holds plaintext
    expiresAt: timestamp('expires_at', { withTimezone: true }).notNull(),
    usedAt: timestamp('used_at', { withTimezone: true }),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  },
  (table) => ({
    klientIdx: index('magic_link_tokens_klient_idx').on(table.klientId),
    tokenHashUq: uniqueIndex('magic_link_tokens_hash_uq').on(table.tokenHash),
  }),
);

// ============================================================================
// SIGNUP REQUESTS — curated B2B sign-up; admin approves and creates klient
// ============================================================================
export const signupRequests = pgTable(
  'signup_requests',
  {
    ...
    company: text('company'),
    role: text('role'), // CMO | Marketing Director | Digital Lead | Iné
    brandSlug: text('brand_slug'), // optional preferred brand
    notes: text('notes'),
    status: text('status').notNull().default('pending'), // pending | approved | rejected
    termsAcceptedAt: timestamp('terms_accepted_at', { withTimezone: true }).notNull(),
    decidedAt: timestamp('decided_at', { withTimezone: true }),
    decidedBy: uuid('decided_by').references(() => klients.id, { onDelete: 'set null' }),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  },
  (table) => ({
...
export const klientsRelations = relations(klients, ({ one, many }) => ({
  brand: one(brands, { fields: [klients.brandId], references: [brands.id] }),
...
export const sessionsRelations = relations(sessions, ({ one }) => ({
  klient: one(klients, { fields: [sessions.klientId], references: [klients.id] }),
}));

export const magicLinkTokensRelations = relations(magicLinkTokens, ({ one }) => ({
  klient: one(klients, { fields: [magicLinkTokens.klientId], references: [klients.id] }),
}));

export const reportsRelations = relations(reports, ({ one }) => ({
  klient: one(klients, { fields: [reports.klientId], references: [klients.id] }),
  brand: one(brands, { fields: [reports.brandId], references: [brands.id] }),
  vertical: one(verticals, { fields: [reports.verticalId], references: [verticals.id] }),
}));
