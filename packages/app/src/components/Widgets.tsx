...
export const Sparkline: FC<{
  ...
  if (!values || values.length === 0)
    return <div style={`color:${C.inkSoft};font-size:12px`}>—</div>;
  const w = 240;
  const h = height;
    ...
    <div style="display:flex;flex-direction:column;gap:6px;height:56px;justify-content:center">
      {rows.map((r) => (
        <div style="display:grid;grid-template-columns:54px 1fr 36px;align-items:center;gap:8px">
          ...
          <Num size={10} color={r.is_klient ? C.signal : C.ink}>
            {r.sov_pct.toFixed(1)}
          </Num>
        </div>
      ))}
...
> = ({ label, value, valueEm, sub, viz, delta, deltaLabel, noRight, noLeft }) => {
  ...
  return (
    <div
      style={`${borders}background:${C.paperPure};padding:20px 22px 18px;display:flex;flex-direction:column;gap:14px;min-height:200px;position:relative`}
    >
      <div style="display:flex;justify-content:space-between;align-items:baseline">
        <MonoLabel size={10} tracking="0.18em">
          {label}
        </MonoLabel>
        <Num size={10} color="rgba(14,17,22,0.4)">
          30D
        </Num>
      </div>
      <div style="display:flex;align-items:baseline;gap:10px">
        <span
          style={`font-family:${C.fontDisplay};font-size:44px;font-weight:400;line-height:1;letter-spacing:-0.03em;color:${valueEm ? C.signal : C.ink};font-style:${valueEm ? 'italic' : 'normal'};font-variant-numeric:tabular-nums`}
        >
          {value}
        </span>
        {sub && (
          <span
            style={`font-family:${C.fontMono};font-size:12px;color:${C.inkSoft};letter-spacing:0.06em`}
          >
            {sub}
          </span>
        )}
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;padding-top:10px;border-top:1px solid rgba(14,17,22,0.1)">
        {delta}
        <Num size={9} color={C.inkSoft}>
          {deltaLabel ?? 'vs minulý mesiac'}
        </Num>
      </div>
    </div>
  );
...
export const LLMCard: FC<{ row: LlmProviderRow }> = ({ row }) => {
  ...
  const bg = `rgba(255,91,58,${0.04 + intensity * 0.1})`;
  const sentiment = row.avg_sentiment_score ?? 0;
  const position = row.avg_position ?? 0;
  return (
    <div
      style={`border:1px solid ${C.ink};background:${C.paperPure};padding:18px 20px;display:flex;flex-direction:column;gap:14px;position:relative;overflow:hidden`}
    >
      <div
        aria-hidden="true"
        style={`position:absolute;inset:0;background:${bg};pointer-events:none`}
      />
      <div style="position:relative;display:flex;flex-direction:column;gap:14px">
        <div style="display:flex;justify-content:space-between;align-items:flex-start">
          <div>
            <span
              style={`font-family:${C.fontDisplay};font-size:18px;font-weight:500;letter-spacing:-0.015em;color:${C.ink}`}
            >
              {name}
            </span>
            <div style="margin-top:2px">
              <Num size={9} color={C.inkSoft}>
                {row.sov_pct.toFixed(0)} % SoV
              </Num>
            </div>
          </div>
          <span
            style={`width:22px;height:22px;border:1px solid ${C.ink};display:inline-flex;align-items:center;justify-content:center`}
          >
        ...
        <div style="display:flex;flex-direction:column;gap:8px">
          <MetricRow
            label="SOV"
            value={`${row.sov_pct.toFixed(1)} %`}
            bar={Math.min(row.sov_pct / 100, 1)}
            primary
          />
    ...
    <div style="display:grid;grid-template-columns:64px 1fr 56px;align-items:center;gap:8px">
      ...
      {bar !== undefined ? (
        <div style={`height:3px;background:${C.bone};position:relative`}>
          <div
            style={`position:absolute;inset:0;width:${Math.max(0, Math.min(1, bar)) * 100}%;background:${color}`}
          />
        </div>
      ) : (
        <div />
      )}
      <Num size={11} color={color}>
        {value}
      </Num>
    </div>
  );
...
export const SoVTrendChart: FC<{ data: SovTrendCell[]; klientSlug: string }> = ({
  data,
  klientSlug,
}) => {
  const W = 920;
  ...
  for (const cell of data) {
    if (!brands.has(cell.brand_slug))
      brands.set(cell.brand_slug, { name: cell.brand_name, points: [] });
  ...
  if (dayList.length < 2) {
    return (
      <div style={`padding:24px;color:${C.inkSoft};font-size:13px`}>Nedostatok dát pre trend.</div>
    );
  }
  ...
  const labelIdxs = Array.from({ length: labelCount }, (_, i) =>
    Math.round((i / (labelCount - 1)) * (dayList.length - 1)),
  );
      ...
      <g transform={`translate(${PAD.l}, ${H - 4})`}>
        {([klient, ...competitors] as const)
          ...
          .map((entry, i) => {
            ...
            const color = isKlient
              ? C.signal
              : (competitorColors[(i === 0 ? 0 : i - 1) % competitorColors.length] ?? C.ink);
            const width = isKlient
              ? 2.4
              : (competitorWidths[(i === 0 ? 0 : i - 1) % competitorWidths.length] ?? 1.5);
            const dash = isKlient
              ? ''
              : (competitorDashes[(i === 0 ? 0 : i - 1) % competitorDashes.length] ?? '');
            return (
              <g transform={`translate(${i * 130}, 0)`}>
                ...
                <text
                  x="22"
                  y="-1"
                  fill={isKlient ? C.signal : C.ink}
                  font-family={C.fontBody}
                  font-size="11"
                  font-weight={isKlient ? 600 : 500}
                >
                  {b.name}
                </text>
              </g>
...
export const TopicHeatmap: FC<{
  ...
  return (
    <div>
      <div
        style={`display:grid;grid-template-columns:100px repeat(${HEATMAP_COLS.length}, 1fr);gap:4px`}
      >
        <span />
        {HEATMAP_COLS.map((c) => (
          <MonoLabel size={9} tracking="0.12em">
            {c.label}
          </MonoLabel>
        ))}
      </div>
      <div style="display:flex;flex-direction:column;gap:4px;margin-top:8px">
        {HEATMAP_ROWS.map((row) => (
          <div
            style={`display:grid;grid-template-columns:100px repeat(${HEATMAP_COLS.length}, 1fr);gap:4px;align-items:stretch`}
          >
            <div style="display:flex;align-items:center">
              <span
                style={`font-family:${C.fontDisplay};font-size:13px;color:${C.ink};letter-spacing:-0.01em`}
              >
                {row.label}
              </span>
            </div>
...
export const CitationList: FC<{
  ...
  if (rows.length === 0) {
    return (
      <div style={`padding:24px 0;color:${C.inkSoft};font-size:13px`}>Žiadne citácie v období.</div>
    );
  }
  return (
    <div style="display:flex;flex-direction:column">
      <div
        style={`display:grid;grid-template-columns:1fr 56px 80px 32px;padding:12px 0;border-bottom:1px solid ${C.ink}`}
      >
      ...
      {rows.map((r) => (
        <div style="display:grid;grid-template-columns:1fr 56px 80px 32px;padding:13px 0;border-bottom:1px solid rgba(14,17,22,0.08);align-items:baseline">
          <span
            style={`font-family:${C.fontDisplay};font-size:15px;font-weight:400;letter-spacing:-0.012em;color:${C.ink}`}
          >
            {r.domain}
          </span>
          <span style="text-align:right">
          ...
          <span
            style={`display:inline-flex;justify-content:flex-end;gap:1px;font-family:${C.fontMono};font-size:12px;color:${C.ink};letter-spacing:0.04em`}
          >
          ...
          <span style="text-align:right">
            <Num
              size={12}
              color={r.trend === 'up' ? C.positive : r.trend === 'down' ? C.negative : C.inkSoft}
            >
...
export const AnomalyCard: FC<{
  ...
  return (
    <div
      style={`border:1px solid ${C.bone};background:${C.paperPure};padding:18px 20px 16px;display:flex;flex-direction:column;gap:12px;border-left:3px solid ${dotColor}`}
    >
      <div style="display:flex;justify-content:space-between;align-items:center">
        <div style="display:inline-flex;align-items:center;gap:10px">
          <span style={`width:8px;height:8px;border-radius:50%;background:${dotColor}`} />
          <MonoLabel size={9} tracking="0.18em" color={dotColor}>
            {sevLabel}
          </MonoLabel>
        ...
        <a
          href="#"
          style={`font-size:12px;color:${C.signal};text-decoration:none;border-bottom:1px solid ${C.signal};padding-bottom:1px`}
        >
          Vyšetriť →
        </a>
      </div>
      <div
        style={`font-family:${C.fontDisplay};font-size:17px;font-weight:400;line-height:1.25;letter-spacing:-0.015em;color:${C.ink}`}
      >
        {title}
      </div>
      <p style={`margin:0;font-size:13px;color:${C.inkSoft};line-height:1.5;max-width:74ch`}>
        {context}
      </p>
      {tags.length > 0 && (
        <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:2px">
          {tags.map((t) => (
            <span
              style={`font-family:${C.fontMono};font-size:9px;letter-spacing:0.12em;color:${C.inkSoft};padding:3px 8px;border:1px solid rgba(14,17,22,0.14)`}
            >
              {t.toUpperCase()}
            </span>
          ))}
...
export function anomalyTitle(a: {
  ...
  if (a.kind === 'sov_drop')
    return `${llm} SoV klesol o ${Math.abs(a.delta).toFixed(1)} pp v kategórii ${cat}.`;
  if (a.kind === 'sov_jump')
    return `${llm} SoV stúpol o ${a.delta.toFixed(1)} pp v kategórii ${cat}.`;
  if (a.kind === 'sentiment_drop')
    return `${llm} sentiment voči vám klesol o ${Math.abs(a.delta).toFixed(2)} v kategórii ${cat}.`;
  return `${llm} · ${cat} · zmena ${a.delta}`;
}

export function anomalyTags(a: {
  provider: string;
  subcategory: string | null;
  kind: string;
}): string[] {
...
export const UpcomingCard: FC<{
  ...
  if (kind === 'coming') {
    pill = (
      <span
        style={`background:${C.signalSoft};color:${C.signal};font-family:${C.fontMono};font-size:9px;letter-spacing:0.14em;padding:4px 8px`}
      >
        BLÍŽI SA
      </span>
    );
    ...
    pill = (
      <span
        style={`background:${C.bone};color:${C.ink};font-family:${C.fontMono};font-size:9px;letter-spacing:0.14em;padding:4px 8px`}
      >
        V PRÍPRAVE
      </span>
    );
    ...
    pill = (
      <span
        style={`background:${C.ink};color:${C.paper};font-family:${C.fontMono};font-size:9px;letter-spacing:0.14em;padding:4px 8px`}
      >
        REZERVOVAŤ
      </span>
    );
  ...
  return (
    <div
      style={`padding:18px 20px;border-top:1px solid ${C.bone};display:flex;flex-direction:column;gap:8px`}
    >
      <div>{pill}</div>
      <div
        style={`font-family:${C.fontDisplay};font-size:17px;font-weight:400;line-height:1.2;color:${C.ink};letter-spacing:-0.015em`}
      >
        {title}
      </div>
      <Num size={10} color={C.inkSoft}>
      ...
      {action && (
        <a
          href="#"
          style={`font-size:12.5px;color:${C.signal};text-decoration:none;border-bottom:1px solid ${C.signal};padding-bottom:1px;align-self:flex-start;margin-top:4px`}
        >
          {action} →
        </a>
      )}
...
// ────────── Section heading ──────────
export const Section: FC<PropsWithChildren<{ title: unknown; subtitle?: unknown }>> = ({
  title,
  subtitle,
  children,
}) => (
  <section style="display:flex;flex-direction:column;gap:14px">
    <header style="display:flex;justify-content:space-between;align-items:baseline;gap:16px;flex-wrap:wrap">
      <h3
        style={`margin:0;font-family:${C.fontDisplay};font-weight:400;font-size:22px;letter-spacing:-0.022em;color:${C.ink};line-height:1.2`}
      >
        {title}
      </h3>
      {subtitle && (
        <div style={`font-size:12px;color:${C.inkSoft};font-family:${C.fontBody}`}>{subtitle}</div>
      )}
    </header>
    {children}
  </section>
